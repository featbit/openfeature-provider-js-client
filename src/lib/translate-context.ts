    ...
    if (evaluationContext.hasOwnProperty(key) && key !== 'targetingKey') {
      ...
      switch (key) {
        ...
        case 'customizedProperties':
          if (
            Array.isArray(value) &&
            value.every((item) => item !== null && typeof item === 'object' && 'name' in item && 'value' in item)
          ) {
            user.customizedProperties = value as unknown as ICustomizedProperty[];
          } else {
            user.customizedProperties = undefined;
          }
          break;
        default:
          featbitLogger.warn(`Unrecognized property '${key}' in EvaluationContext`);
          break;
      }
    }
  }
